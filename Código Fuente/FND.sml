fun impr_as_vals [] n             = ")"
|   impr_as_vals ((v,b) :: vbs) n = 
	if n > 1 then (if b then v ^ " :&&: " ^ impr_as_vals vbs (n-1) else "~:" ^ v  ^ " :&&: " ^ impr_as_vals vbs (n-1))
	else (if b then v ^ impr_as_vals vbs (n-1) else "~:" ^ v  ^ impr_as_vals vbs (n-1))
...
fun tabla prop =
    let
    	...
    	fun imprimir_fila vars_bools es_verdadero isPrimero =
			case isPrimero of
			   0 => print( if es_verdadero then " :||: (" ^ impr_as_vals vars_bools n else "") 
			 | 1 => print( if es_verdadero then "(" ^ impr_as_vals vars_bools n else "") 
